...
var deathAnimation = function(){
	...
	if(splash){
		ctx.drawImage(splash, width / 2 - 218, height / 2 - 180);
		splash = undefined;
	}
	else {
        ctx.drawImage(scoreBoard, width / 2 - 218, height / 2 - 160);
...
//	ctx.rotate(deg);
	ctx.drawImage(bird, 0, birdN * 19, bird.width, bird.height / 4, birdPos, birdY, bird.width, bird.height / 4);
//	ctx.rotate(-deg);
//	ctx.translate(-width * 0.35 - 17, -birdY - 12);
	birdF = (birdF + 1) % 6;
	if(birdF % 6 == 0)
